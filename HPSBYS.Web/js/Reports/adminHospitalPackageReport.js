...
function getPackageBlockReport() {
    var param = {
        ...
        "StateCode": $('#ddState1').val(), "DistrictCode": $('#ddDistrict1').val(), "HospitalCode": $('#ddHospital1').val()
    };
    param = JSON.stringify(param);
    $.ajax({
        url: ServiceURL + "/api/Report/adminHospitalPackageBlockReport",
        type: "POST",
        data: param,
...
function bindgrdPackageBlock(rData) {
    ...
    $("#grdPackageBlock").DataTable({
        ...
        columns: [
            ...
            {
                title: '<div class="prehed-bold"> Member Details </div><div class="prehed-small"> ID | Name | URN </div>',
                align: "left",
                className: "hiddenExport",
                render: function (data, type, row, meta) {
                    var memberDetails = '<b class="text-success">' + row.memberId + '</b>' + '<div class="pre-bold">' + row.memberName
                        + '</div><div><span class="pre-small">' + row.urn + '</span></div>';
                    return memberDetails;
                }
            },
            { data: "urn", type: "text", title: "URN", visible: false },
            { data: "memberName", type: "text", title: "Member Name", visible: false },
            { data: "memberId", type: "text", title: "Member Id", visible: false },
            {
                title: '<div class="prehed-bold"> Package Details </div><div class="prehed-small"> Code | Name |</div><div class="prehed-small"> Is Pre-Auth </div>',
                align: "left",
                className: "hiddenExport",
                render: function (data, type, row, meta) {
                    return '<b class="text-success">' + row.procedureCode + '</b> <div class="pre-bold">' + row.packageName
                        + '</div><div class="badge badge-success badge-font mt-1">' + row.isPreAuth + '</div>';
                }
            },
...
function getPackageUnblockReport() {
    var param = {
        ...
        "StateCode": $('#ddState2').val(), "DistrictCode": $('#ddDistrict2').val(), "HospitalCode": $('#ddHospital2').val()
    };
    param = JSON.stringify(param);
    $.ajax({
        url: ServiceURL + "/api/Report/adminHospitalPackageUnblockReport",
        type: "POST",
        data: param,
        contentType: 'application/json',
        beforeSend: function () {
...
function getPackageDischargeReport() {
    var param = {
        ...
        "StateCode": $('#ddState3').val(), "DistrictCode": $('#ddDistrict3').val(), "HospitalCode": $('#ddHospital3').val()
    };
    param = JSON.stringify(param);
    $.ajax({
        url: ServiceURL + "/api/Report/adminHospitalPackageDischargeReport",
        type: "POST",
        data: param,
        contentType: 'application/json',
        beforeSend: function () {
