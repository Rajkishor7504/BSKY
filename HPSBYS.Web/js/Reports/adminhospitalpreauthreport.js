        ...
        success: function (response) {
            ...
            $("#grdPreAuthReportDetails").DataTable({
                ...
                columns: [
                    ...
                    { data: "package", type: "text", title: "Package Name <div class='prehed-small'> Code </div>", align: "left" },
                    {
                        type: "text", title: "PreAuth Amount <div class='prehed-small'> Date & Time </div>", align: "left", className: "hiddenExport",
                        ...
                        type: "text", title: "Status", align: "left", render: function (data, type, row, meta) {
                            switch (row.status) {
                                ...
                                case "UNBLOCKED (AUTO-APPROVED)":
                                    return '<span class="badge badge-warning badge-font"  style="background:#1393d6; color: #fff;">' + row.status + '</span></div>';
...
function getActionTakenByUser() {
    var params = { Action: 'SU' };
    $.ajax({
        url: ServiceURL + "/api/Common/GetAllActionTakenByUser",
        type: 'GET',
        data: params,
        contentType: 'application/json; charset=UTF-8',
        success: function (result) {
            $.each(result, function (i, data) {
                $("#ddActionTakenBy").append('<option value=' + data.userId + '>' + data.actionTakenBy + '</option>');
            });
        },
